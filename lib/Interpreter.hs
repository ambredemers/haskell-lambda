...
evalTerm (TBVar index _ dbg) = do
  ...
  if index < length stack
    ...
    else throwError $ makeErrorString input dbg message
  where
...
evalTerm (TApp fn args dbg) = do
  ...
  args' <- evalTerms args
  apply fn' args' dbg
evalTerm (TLet value body _ dbg) = do value' <- evalTerm value; localContext (value' :) (evalTerm body)
...
apply abs@(TAbs arity body env _ _) args dbg
  ...
  | otherwise = do EvalContext _ input <- ask; throwError $ makeErrorString input dbg message
  where
